...
class MainWindowAPI(QMainWindow):
    ...
    def handle_step_movement(self, direction, step_size):
        ...
        target_pan = raw_pan
        target_tilt = raw_tilt
        
        if direction == 'up':
            target_tilt += step_size
        elif direction == 'down':
            target_tilt -= step_size
        elif direction == 'left':
            target_pan -= step_size
        elif direction == 'right':
            target_pan += step_size
        
        # Start the movement
        if not self.api_client.move(direction, speed=0x10):
            QMessageBox.warning(self, "Error", f"Failed to start movement: {self.api_client.last_error}")
        ...
        # Start movement timer to stop after specified time
        if self.movement_timer is not None:
            self.movement_timer.cancel()
        
        self.movement_timer = threading.Timer(movement_time, self.timed_stop)
        self.movement_timer.daemon = True
        self.movement_timer.start()
        
        # Show status message
        self.statusBar.showMessage(f"Moving {direction} for {movement_time}s with step size {step_size}°", 2000)
    
    def timed_stop(self):
        """Stop movement after timer expires"""
        if self.is_moving:
    ...
    def stop_movement(self):
        ...
        # Cancel movement timer if it's running
        if self.movement_timer is not None:
            self.movement_timer.cancel()
            self.movement_timer = None
